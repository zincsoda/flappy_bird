...
class Record:
    ...
    def draw_record(self, canvas):
        canvas.draw_text('Best Record:' + str(self.record), [80, 190], 20, 'White', 'monospace')
...
class Game:
    ...
    def is_game_over(self, canvas):
        if self.game_over:
            self.score.draw_score_board(canvas)
            self.record.draw_record(canvas)
            ...
            if self.pipe_creator.pipes:
                ...
                if first_pipe.center_x + PIPE_HEAD_WIDTH / 2.0 < self.bird.center_x - BIRD_WIDTH / 2.0:
                    if not first_pipe.is_passed:
                        ...
                        self.record.update_record(self.score.score)
